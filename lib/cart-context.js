...
export function CartProvider({ children }) {
  ...
  useEffect(() => {
    try {
      ...
      // Age check from session cookie (browser-close = reset)
  ...
  const confirmAge = () => {
    // Session-only cookie — cleared when browser closes. NMI gateway compliant.
    setAgeVerified(true)
    if (typeof window !== 'undefined') {
      try {
        // No "expires" → session cookie, dies on browser close
        document.cookie = `${AGE_COOKIE}=true; path=/; SameSite=Lax; Secure`
      } catch {}
    }
  }
  const declineAge = () => {
