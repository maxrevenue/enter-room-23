...
import { PACKAGING, buildOrderSubject } from '../customer-copy.mjs'

export { buildOrderSubject }
...
export function buildOrderEmailContent({ orderId, items, totals, splitFulfillment }) {
  ...
  const splitLine = splitFulfillment
    ? 'Some items will ship separately. Each package is plain and unlabeled. The exterior does not name what is inside.'
...
<body style="font-family:Georgia,serif;color:#222;background:#fff;margin:0;padding:24px;">
  ...
  <p>${escapeHtml(PACKAGING.statement)}</p>
