...
/** Homepage + admin seed default until a product document flag is set. House lube only on storefront. */
export const PRODUCT_OF_THE_MONTH_ID = DEFAULT_HOUSE_LUBE_ID
...
export function getCollection(slug) {
  ...
  if (COLLECTIONS[resolved]) return COLLECTIONS[resolved]
  const title = titleFromSlug(resolved)
...
export function searchProducts(query = '', { collection, category, minPrice, maxPrice } = {}) {
  ...
  if (collection === 'new-arrivals') {
    ...
    results = results.filter((product) => productMatchesCollection(product, collection))
  }
